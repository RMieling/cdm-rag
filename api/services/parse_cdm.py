...
async def fetch_and_traverse_manifest(
    ...
    """
    Function that recursively traverses through the manifests and resolves the entities in each of them through the CDM SDK
    """

    corpus, _ = setup_cdm_corpus()
    manifest_path = f"local:{str(manifest_name)}"
    manifest_resolved_name = manifest_name.split("/")[-1].replace(".manifest.cdm.json", "_with_relationships.manifest.cdm.json")
    ...
    for entity in tqdm.tqdm(manifest.entities, disable=not show_progress):
        ...
        if load_cached_resolved:
            ...
            resolved_entity = await wrt_entity.create_resolved_entity_async(resolved_entity_name.replace(".cdm.json", ""), res_opt)
...
async def ensure_manifest_resolved(traverse_manifest_files: dict):
    ...
    for manifest_path, traverse_submanifests in traverse_manifest_files.items():
        ...
        parse_cdm_logger.info(f"Resolved schemas not found for {original_filename}. Generating now (this may take a moment)...")
        try:
            await fetch_and_traverse_manifest(
