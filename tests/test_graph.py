...
def test_financial_account_has_inherited_attribute(neo4j_driver):
    ...
    with neo4j_driver.session() as session:
        result = session.run(
            query, entity_name=target_entity_name, path_substring=target_path_substring, attr_name=expected_attribute
        )

        record = result.single()
        assert record["has_attribute"] is True, (
            f"Failed: {target_entity_name} ({target_path_substring}) cannot reach '{expected_attribute}'."
        )
